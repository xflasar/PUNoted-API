...
async def get_vendor_stores(request: Request):
    try:
        ...
        vendors_data = await fetch_public_vendors(request.app.state.db)
        return JSONResponse(
            status_code=200,
            content={"success": True, "vendors": vendors_data},
        )
    except Exception as e:
        logger.error(f"Failed to fetch vendor stores: {e}", exc_info=True)
        return JSONResponse(status_code=500, content={"success": False, "message": "Server error."})
