...
async def handle_accounting_currency_balance_data_message(db: Any, raw_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not converted_data:
        logger.debug("No accounting records in payload for processing. Exiting.")
    ...
    try:
        ...
        if user_response and user_response.get("userdataid") is not None:
            ...
            logger.warning(f"User with accountid {raw_payload['userId']} not found for accounting data processing.")
        ...
        async with db.pool.acquire() as con:
            async with con.transaction():
                try:
                    ...
                    logger.debug(f"Successfully UPSERTed {len(records_for_upsert)} accounting balance records.")
                except Exception as e:
