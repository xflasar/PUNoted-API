...
async def handle_site_available_reserve_population_data_message(db: Any, raw_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug("Starting processing site reserve pop data.")
    ...
    try:
        if not population_data_records:
            logger.debug("No reserve population data to process.")
            return {
                "success": True,
                "message": "No reserve population data to process.",
            }

        planetid = await db.fetch_one(
        ...
        logger.debug(
            f"Finished processing site reserve pop data for {len(population_data_records)} records in {duration:.2f} ms."
        )
        return {
            "success": True,
            "message": f"Successfully processed {len(population_data_records)} reserve population records.",
        }
