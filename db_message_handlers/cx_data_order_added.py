...
async def handle_comex_order_added_message(db: Database, raw_payload: Dict[str, Any]) -> Dict[str, Any]:
    start_time = time.perf_counter()
    logger.debug("Starting processing comex order added data.")

    record = raw_payload.get("data")
    if not record:
        logger.debug("No comex order added record in payload. Exiting.")
