...
async def handle_cx_broker_data_message(db: Database, raw_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug("Starting processing cx_broker data.")

    converted_data = raw_payload.get("data")
    if not converted_data:
        logger.debug("No cx_broker records in payload. Exiting.")
    ...
    try:
        ...
        for entry in cx_broker_data_to_upsert:
            entry["xata_updatedat"] = current_time
        ...
        async with db.pool.acquire() as con:
            async with con.transaction():
                ...
                await _delete_and_insert(
                    con,
                    "cx_brokers_buy_orders",
                    buy_orders_to_insert,
                    "brokermaterialid",
                    brokermaterialid,
                )
                ...
                await _delete_and_insert(
                    con,
                    "cx_brokers_sell_orders",
                    sell_orders_to_insert,
                    "brokermaterialid",
                    brokermaterialid,
                )
    ...
    logger.debug(f"Processing cx_broker records took {end_time - start_time:.4f} seconds")

    return {"success": True, "message": "Processed cx_broker record successfully."}
...
async def _delete_and_insert(
    ...
    if records:
        ...
        try:
            ...
            logger.error(f"Database error during UPSERT: {e}", exc_info=True)
            raise
    logger.debug(f"Finished processing {len(records)} records for {table_name}.")
