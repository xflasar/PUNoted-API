...
async def handle_sectors_message(db, raw_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.debug("Starting processing batch of sector data.")
    ...
    if not sectors_data:
        logger.debug("No sectors found in payload. Exiting.")
    ...
    try:
        async with db.pool.acquire() as con:
            async with con.transaction():
                ...
                logger.debug(f"Successfully UPSERTed {len(sectors_to_upsert)} sectors.")
                ...
                logger.debug(f"Successfully UPSERTed {len(subsectors_to_upsert)} subsectors.")
                ...
                logger.debug(f"Successfully UPSERTed {len(vertices_to_upsert)} vertices.")

            end_time = time.perf_counter()
            logger.debug(f"Total processing for the entire batch took {end_time - start_time:.4f} seconds.")
            return {
                "success": True,
                "message": "Batch of sector data processed successfully.",
            }
