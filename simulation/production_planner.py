...
HOUSING_BUILDING: Dict[str, List[HousingWorkforce]] = {
    "HB1": [HousingWorkforce(WorkforceTypeTicker="PIONEER", capacity=100.0)],
    "HB2": [HousingWorkforce(WorkforceTypeTicker="SETTLER", capacity=100.0)],
    "HB3": [HousingWorkforce(WorkforceTypeTicker="TECHNICIAN", capacity=100.0)],
    "HB4": [HousingWorkforce(WorkforceTypeTicker="ENGINEER", capacity=100.0)],
    "HB5": [HousingWorkforce(WorkforceTypeTicker="SCIENTIST", capacity=100.0)],
    "HBB": [
    ...
    for site_instance in state.company.sites:
        ...
        if not isinstance(site_instance, Site):
            simulation_log.append(
                f"  WARNING: Expected Site object but got {type(site_instance)} in company.sites.Skipping."
            )
            continue
        ...
        for building_instance in site_instance.Buildings.values():
            ...
            if not isinstance(building_instance, BuildingInstance):
                simulation_log.append(
                    f"    WARNING: Expected BuildingInstance object but got {type(building_instance)} in site {site_instance.SiteId}. Skipping this building."
                )
                continue
            ...
            if not building_def:
                simulation_log.append(
                    f"    WARNING: Building definition for {building_instance.BuildingTicker} not found in static data."
                )
                continue
            ...
            for order in production_orders:
                if not order.IsHalted and order.CompletionEpochMs is not None:
                    ...
                    if not recipe:
                        simulation_log.append(
                            f"      WARNING: Recipe not found for standard recipe name: {order.StandardRecipeName}"
                        )
                        continue
                    ...
                    for output in recipe.Outputs:
                        if output.MaterialTicker == material_ticker:
                            if order.DurationMs > 0:
                                ...
                                simulation_log.append(
                                    f"      Order {order.ProductionLineOrderId}: Producing {production_per_day:.2f} {material_ticker}/day from {building_def.name} on site {site_instance.SiteId}"
                                )
                            else:
                                simulation_log.append(
                                    f"      WARNING: Production order {order.ProductionLineOrderId} has 0 duration, skipping rate calculation."
                                )

    if target_site_id:
        simulation_log.append(
            f"--- Total current production of {material_ticker} on site {target_site_id}: {total_production_units_per_day:.2f} units/day ---"
        )
    else:
        simulation_log.append(
            f"--- Total current production of {material_ticker} across all sites: {total_production_units_per_day:.2f} units/day ---"
        )
...
def find_best_recipe_and_building(
    ...
    if preferred_recipe_ticker and preferred_building_ticker:
        ...
        if preferred_recipe and preferred_building_def:
            ...
            if (
                any(output.MaterialTicker == desired_material_ticker for output in preferred_recipe.Outputs)
                and preferred_recipe.BuildingTicker == preferred_building_ticker
            ):
...
def _get_material_production_category(
    ...
    if not producing_building_tickers:
        simulation_log.append(
            f"  Could not find any recipes producing {material_ticker}. Cannot determine its production category."
        )
        return None
    ...
    for b_ticker in producing_building_tickers:
        ...
        if building_def and hasattr(building_def, "expertise") and building_def.expertise:
            category = (
                building_def.expertise.upper()
            )  # Assuming Expertise is already in a suitable format like "AGRICULTURE"
            simulation_log.append(
                f"  {material_ticker} is produced by {b_ticker}. Category: {category} (from building expertise)."
            )
            return category

    simulation_log.append(
        f"  WARNING: Could not determine COGC category for material {material_ticker} from building expertise. No specific COGC bonus will be considered."
    )
...
def _get_building_expertise_category(
    ...
    if building_def and hasattr(building_def, "expertise") and building_def.expertise:
        ...
        simulation_log.append(
            f"  WARNING: Could not determine COGC category for building {building_ticker} from its expertise. No specific COGC bonus will be considered."
        )
        return None
...
def find_best_expansion_planet(
    ...
    """
    simulation_log.append(
        f"--- Searching for best expansion planet for production type: {desired_production_type} (Building: {building_ticker_for_resource_check}) ---"
    )
    ...
    for planet in all_planets_data:
        ...
        if (
            desired_production_type
        ):  # Only try to form expected_cogc_program_type if desired_production_type is not None
        ...
        if expected_cogc_program_type:  # Only iterate if we have a valid program type to look for
            for program in planet.COGCPrograms:
                if (
                    program.ProgramType == expected_cogc_program_type
                    and program.StartEpochMs <= current_epoch_ms <= program.EndEpochMs
                    and planet.COGCProgramStatus == "ACTIVE"
                ):
                    has_active_preferred_program = True
                    simulation_log.append(
                        f"  Planet {planet.PlanetName} ({planet.PlanetId}) has an ACTIVE COGC program: {program.ProgramType}."
                    )
                    break
        ...
        if desired_production_type == "AGRICULTURE":
            if planet.Fertility > 0:  # Must have positive fertility for agriculture
                is_suitable = True
                simulation_log.append(
                    f"  Planet {planet.PlanetName} ({planet.PlanetId}) is suitable for AGRICULTURE (Fertility: {planet.Fertility:.2f})."
                )
            else:
                simulation_log.append(
                    f"  Planet {planet.PlanetName} ({planet.PlanetId}) is NOT suitable for AGRICULTURE (Fertility: {planet.Fertility:.2f})."
                )
        ...
        # Check Resources for Resource Extraction (now more precise)
            ...
            if building_ticker_for_resource_check:
                ...
                if extracted_material_ticker:
                    extracted_material_id = next(
                        (
                            m.MaterialId
                            for m_id, m in static_materials.items()
                            if m.MaterialTicker == extracted_material_ticker
                        ),
                        None,
                    )
                    if extracted_material_id:
                        ...
                        for resource in planet.Resources:
                            if resource.MaterialId == extracted_material_id and resource.Factor > 0:
                                ...
                                if (
                                    (building_ticker_for_resource_check == "RIG" and resource.ResourceType == "LIQUID")
                                    or (
                                        building_ticker_for_resource_check == "EXT"
                                        and resource.ResourceType == "MINERAL"
                                    )
                                    or (
                                        building_ticker_for_resource_check == "COL"
                                        and resource.ResourceType == "GASEOUS"
                                    )
                                ):
                                    is_suitable = True
                                    simulation_log.append(
                                        f"  Planet {planet.PlanetName} ({planet.PlanetId}) is suitable for {building_ticker_for_resource_check} (extracts {extracted_material_ticker}, Factor: {resource.Factor:.2f})."
                                    )
                                    resource_found = True
                                    break
                        if not resource_found:
                            simulation_log.append(
                                f"  Planet {planet.PlanetName} ({planet.PlanetId}) does NOT have required resource {extracted_material_ticker} for {building_ticker_for_resource_check} with positive factor or correct type."
                            )
                            continue
                    else:
                        simulation_log.append(
                            f"  Material ID not found for extracted ticker: {extracted_material_ticker}. Cannot check specific resources."
                        )
                        continue
                else:
                    simulation_log.append(
                        f"  Could not determine extracted material for building {building_ticker_for_resource_check}. Falling back to general resource check."
                    )
                    # Fallback to general check if specific material not found for building
                    if any(r.Factor > 0 for r in planet.Resources):
                        is_suitable = True
                        simulation_log.append(
                            f"  Planet {planet.PlanetName} ({planet.PlanetId}) has some resources and is generally suitable for RESOURCE_EXTRACTION."
                        )
                    else:
                        simulation_log.append(
                            f"  Planet {planet.PlanetName} ({planet.PlanetId}) has no resources and is NOT suitable for RESOURCE_EXTRACTION."
                        )
                        continue
        ...
        # For other production types (e.g., METALURGY, CHEMICAL), prioritize COGC programs and general industrial infrastructure
        elif desired_production_type in [
            "METALURGY",
            "REFINING",
            "CHEMICAL",
            "FOOD_PROCESSING",
            ...
            if has_active_preferred_program:
                ...
                simulation_log.append(
                    f"  Planet {planet.PlanetName} ({planet.PlanetId}) is generally suitable for {desired_production_type} (has local market/chamber of commerce)."
                )
            else:
                simulation_log.append(
                    f"  Planet {planet.PlanetName} ({planet.PlanetId}) is not ideal for {desired_production_type} (lacks COGC bonus and general industrial infrastructure)."
                )
            ...
            is_suitable = True
            simulation_log.append(
                f"  Planet {planet.PlanetName} ({planet.PlanetId}) is considered generally suitable (production type not specific or recognized)."
            )
    ...
    if candidate_planets:
        ...
        if expected_cogc_program_type:  # Only filter if we have a valid program type to look for
            preferred_cogc_planets = [
                ...
                if any(
                    prog.ProgramType == expected_cogc_program_type
                    and prog.StartEpochMs <= current_epoch_ms <= prog.EndEpochMs
                    and p.COGCProgramStatus == "ACTIVE"
                    for prog in p.COGCPrograms
                )
            ]
        ...
        if preferred_cogc_planets:
            ...
            simulation_log.append(
                f"--- Best expansion planet found (COGC-preferred): {best_planet.PlanetName} ({best_planet.PlanetId}) ---"
            )
            return best_planet
        else:
            best_planet = random.choice(candidate_planets)  # Pick randomly from general suitable
            simulation_log.append(
                f"--- Best expansion planet found (general suitable): {best_planet.PlanetName} ({best_planet.PlanetId}) ---"
            )
            return best_planet
    else:
...
def _calculate_material_cost_and_source(
    ...
    if recursion_depth > max_recursion_depth:
        simulation_log.append(
            f"  {'  ' * recursion_depth}Recursion depth limit reached for {material_ticker}. Defaulting to market buy price."
        )
        # Fallback to market price if recursion limit is hit
        market_price = next(
            (md.PriceAverage for md in state.dynamic_market_data if md.MaterialTicker == material_ticker),
            None,
        )
    ...
    simulation_log.append(
        f"  {'  ' * recursion_depth}--- Analyzing cost for {material_ticker} (Depth: {recursion_depth}) ---"
    )

    # Option 1: Buy from Market
    buy_cost_per_unit = float("inf")
    market_data = next(
        (md for md in state.dynamic_market_data if md.MaterialTicker == material_ticker),
        None,
    )
    if market_data:
        if market_data.Ask is not None:
            ...
            simulation_log.append(
                f"  {'  ' * recursion_depth}Market Ask price for {material_ticker}: {buy_cost_per_unit:.2f} ICA/unit."
            )
        elif market_data.Bid is not None:
            buy_cost_per_unit = market_data.Bid
            simulation_log.append(
                f"  {'  ' * recursion_depth}Market Bid price for {material_ticker}: {buy_cost_per_unit:.2f} ICA/unit."
            )
        elif market_data.PriceAverage is not None:
            buy_cost_per_unit = market_data.PriceAverage
            simulation_log.append(
                f"  {'  ' * recursion_depth}Market Average price for {material_ticker}: {buy_cost_per_unit:.2f} ICA/unit."
            )
        else:
            simulation_log.append(
                f"  {'  ' * recursion_depth}No usable market data (Ask, Bid, Average) found for {material_ticker}. Cannot buy."
            )
    else:
    ...
    # Option 2: Produce
    possible_recipes = [
        r for r in state.static_recipes.values() if any(o.MaterialTicker == material_ticker for o in r.Outputs)
    ]
    ...
    for recipe in possible_recipes:
        simulation_log.append(
            f"  {'  ' * recursion_depth}Evaluating recipe: {recipe.RecipeName} (Building: {recipe.BuildingTicker})"
        )
        ...
        if planet_for_this_production is None:
            ...
            material_category = _get_material_production_category(
                material_ticker,
                state.static_recipes,
                state.static_buildings,
                simulation_log,
            )

            if not state.all_planets_data:
                simulation_log.append(
                    f"  {'  ' * recursion_depth}No planet data available. Cannot assign planet for {material_ticker} production. Skipping recipe {recipe.RecipeName}."
                )
            ...
            if best_new_planet_data:
                ...
                simulation_log.append(
                    f"  {'  ' * recursion_depth}Determined new planet {best_new_planet_data.PlanetName} ({planet_for_this_production}) for {material_ticker} production."
                )
            else:
                simulation_log.append(
                    f"  {'  ' * recursion_depth}Could not find a suitable planet for {material_ticker} production. Skipping recipe {recipe.RecipeName}."
                )
        ...
        for input_item in recipe.Inputs:
            ...
            (
                ...
                input_source_type,
                input_sub_buildings,
                input_production_planet_id,
            ) = _calculate_material_cost_and_source(
            ...
            if input_cost == float("inf"):
                simulation_log.append(
                    f"  {'  ' * (recursion_depth + 1)}WARNING: Cannot acquire input {input_item.MaterialTicker}. Skipping recipe {recipe.RecipeName}."
                )
            ...
            for sub_bldg in input_sub_buildings:
                if sub_bldg.planet_id is None:  # If not already assigned by a deeper raw material search
                    sub_bldg.planet_id = (
                        input_production_planet_id if input_production_planet_id else planet_for_this_production
                    )
        ...
        if not building_def:
            simulation_log.append(
                f"  {'  ' * recursion_depth}WARNING: Building definition for {recipe.BuildingTicker} not found. Skipping recipe."
            )
            continue
        ...
        for cost_item in building_cost_items:
            ...
            cost_market_data = next(
                (md for md in state.dynamic_market_data if md.MaterialTicker == cost_item.MaterialTicker),
                None,
            )
            item_price = 0.0
            if cost_market_data:
        ...
        if planet_for_this_production:  # Use the determined planet for COGC check
            planet_data_for_cogc = next(
                (pd for pd in state.all_planets_data if pd.PlanetId == planet_for_this_production),
                None,
            )
                ...
                if material_category:  # Only apply bonus if a category was determined
                    ...
                    for program in planet_data_for_cogc.COGCPrograms:
                        if (
                            program.ProgramType == expected_cogc_program_type
                            and program.StartEpochMs <= current_epoch_ms <= program.EndEpochMs
                            and planet_data_for_cogc.COGCProgramStatus == "ACTIVE"
                        ):
                            # Apply a hypothetical bonus, e.g., 10% faster production
                            effective_duration_ms *= 0.90  # 10% faster
                            simulation_log.append(
                                f"  {'  ' * recursion_depth}Applied COGC bonus for {material_category} on {planet_data_for_cogc.PlanetName}. New effective duration: {effective_duration_ms:.2f}ms."
                            )
                            break
        ...
        if output_amount > 0 and effective_duration_ms > 0:
            ...
            conceptual_building_cost_per_unit = building_construction_cost / (
                output_amount * (MS_IN_DAY / effective_duration_ms) * 365
            )  # Amortize over 1 year of production
            cost_per_unit_produced += conceptual_building_cost_per_unit

            simulation_log.append(
                f"  {'  ' * recursion_depth}Production cost for {material_ticker} via {recipe.RecipeName}: {cost_per_unit_produced:.2f} ICA/unit (includes conceptual building cost)."
            )
            ...
            simulation_log.append(
                f"  {'  ' * recursion_depth}WARNING: Recipe {recipe.RecipeName} has zero output amount or duration. Skipping."
            )
    ...
    if is_final_material:
        if best_production_recipe:
            simulation_log.append(
                f"  {'  ' * recursion_depth}Decision for {material_ticker}: FORCING PRODUCE (as it's the final desired material) via {best_production_recipe.RecipeName} (Cost: {best_production_cost_per_unit:.2f})"
            )
            ...
            return (
                best_production_cost_per_unit,
                "PRODUCE",
                best_sub_buildings,
                best_production_planet_id,
            )
        else:
            simulation_log.append(
                f"  {'  ' * recursion_depth}ERROR: Cannot produce final desired material {material_ticker}. No suitable recipe found."
            )
        ...
        if buy_cost_per_unit <= best_production_cost_per_unit:
            simulation_log.append(
                f"  {'  ' * recursion_depth}Decision for {material_ticker}: BUY (Cost: {buy_cost_per_unit:.2f})"
            )
            return buy_cost_per_unit, "BUY", [], []  # No sub-buildings needed if buying
        else:
            simulation_log.append(
                f"  {'  ' * recursion_depth}Decision for {material_ticker}: PRODUCE via {best_production_recipe.RecipeName} (Cost: {best_production_cost_per_unit:.2f})"
            )
            # Add the main production building to the sub-buildings list
            if best_production_building_ticker:
                best_production_building_def = state.static_buildings.get(best_production_building_ticker)
...
def run_simulation(
    ...
    simulation_log: List[str] = []
    simulation_log.append(
        f"Starting simulation for {desired_material_ticker} with target rate {target_production_rate:.2f} units/day."
    )
    ...
    if production_gap > 0:
        simulation_log.append(
            f"Production gap detected. Determining buildings required to produce {desired_material_ticker}."
        )
        ...
        (
            ...
            source_type,
            chain_buildings,
            production_planet_id_for_top_level,
        ) = _calculate_material_cost_and_source(
        ...
        if source_type == "PRODUCE":
            simulation_log.append(
                f"  Optimal strategy for {desired_material_ticker}: PRODUCE. Estimated cost: {final_cost_per_unit:.2f} ICA/unit (per unit of {desired_material_ticker})."
            )
            ...
            if top_level_producing_recipe and top_level_producing_building_ticker:
                output_material = next(
                    (
                        output
                        for output in top_level_producing_recipe.Outputs
                        if output.MaterialTicker == desired_material_ticker
                    ),
                    None,
                )
                if output_material and top_level_producing_recipe.DurationMs > 0:
                    production_rate_per_building = (
                        output_material.Amount / top_level_producing_recipe.DurationMs
                    ) * MS_IN_DAY
                else:
                    simulation_log.append(
                        f"  WARNING: Top-level production recipe for {desired_material_ticker} has zero output or duration. Cannot scale production chain."
                    )
                    production_rate_per_building = 0.0
            else:
                simulation_log.append(
                    f"  WARNING: Could not find top-level production recipe for {desired_material_ticker}. Cannot scale production chain."
                )
                production_rate_per_building = 0.0
            ...
            if production_rate_per_building > 0:
                ...
                simulation_log.append(
                    f"  Calculated {num_chain_sets_to_build} sets of production chain needed to meet the gap."
                )
                ...
                for sub_bldg_template in chain_buildings:  # Iterate through each unique building type needed
                    ...
                    if building_area_per_unit is None:
                        simulation_log.append(
                            f"  WARNING: Building area not defined for {sub_bldg_template.building_ticker}. Skipping area check for this building."
                        )
                    ...
                    while remaining_amount_to_place > 0:
                        ...
                        building_cogc_category = _get_building_expertise_category(
                            sub_bldg_template.building_ticker,
                            initial_state.static_buildings,
                            simulation_log,
                        )
                        ...
                        if building_cogc_category:
                            for (
                                planet_id_candidate,
                                planet_rec_candidate,
                            ) in planet_recommendations_map.items():
                                ...
                                for existing_bldg in planet_rec_candidate.recommended_buildings_on_planet:
                                    existing_bldg_cogc = _get_building_expertise_category(
                                        existing_bldg.building_ticker,
                                        initial_state.static_buildings,
                                        simulation_log,
                                    )
                                ...
                                if has_matching_expertise:
                                    current_used_area_on_planet = sum(
                                        ...
                                        if initial_state.static_buildings.get(b.building_ticker)
                                        and initial_state.static_buildings.get(b.building_ticker).area is not None
                                    )
                                    max_total_area_on_planet = (
                                        BASE_MAX_AREA_FIRST_PERMIT
                                        + (MAX_ADDITIONAL_PERMITS * ADDITIONAL_AREA_PER_PERMIT)
                                        - SITE_BASE_AREA_COST
                                    )
                                    units_that_fit_in_remaining_area = 0
                                    if building_area_per_unit > 0:
                                        units_that_fit_in_remaining_area = math.floor(
                                            (max_total_area_on_planet - current_used_area_on_planet)
                                    ...
                                    if units_that_fit_in_remaining_area > 0:
                                        ...
                                        simulation_log.append(
                                            f"  Prioritizing existing recommended planet {current_planet_rec.planet_name} ({chosen_planet_id_for_building}) for {sub_bldg_template.building_name} due to matching COGC expertise ({building_cogc_category}) and available space."
                                        )
                        ...
                        if chosen_planet_id_for_building is None:
                            ...
                            for planet_candidate in available_planets:
                                ...
                                if building_cogc_category == "AGRICULTURE" and planet_candidate.Fertility > 0:
                                    ...
                                    if extracted_material_ticker:
                                        extracted_material_id = next(
                                            (
                                                m.MaterialId
                                                for m_id, m in initial_state.static_materials.items()
                                                if m.MaterialTicker == extracted_material_ticker
                                            ),
                                            None,
                                        )
                                        if extracted_material_id:
                                            for resource in planet_candidate.Resources:
                                                if resource.MaterialId == extracted_material_id and resource.Factor > 0:
                                                    if (
                                                        (
                                                            sub_bldg_template.building_ticker == "RIG"
                                                            and resource.ResourceType == "LIQUID"
                                                        )
                                                        or (
                                                            sub_bldg_template.building_ticker == "EXT"
                                                            and resource.ResourceType == "MINERAL"
                                                        )
                                                        or (
                                                            sub_bldg_template.building_ticker == "COL"
                                                            and resource.ResourceType == "GASEOUS"
                                                        )
                                                    ):
                                                        is_planet_suitable_for_building = True
                                                        break
                                    if (
                                        not is_planet_suitable_for_building
                                    ):  # If specific resource not found or not suitable
                                        simulation_log.append(
                                            f"  Planet {planet_candidate.PlanetName} ({planet_candidate.PlanetId}) is NOT suitable for {sub_bldg_template.building_ticker} (missing specific resource or type)."
                                        )
                                        continue
                                elif building_cogc_category in [
                                    "METALURGY",
                                    "REFINING",
                                    "CHEMICAL",
                                    "FOOD_PROCESSING",
                                    ...
                                    if building_cogc_category:
                                        ...
                                        for program in planet_candidate.COGCPrograms:
                                            if (
                                                program.ProgramType == expected_cogc_program_type
                                                and program.StartEpochMs <= current_epoch_ms <= program.EndEpochMs
                                                and planet_candidate.COGCProgramStatus == "ACTIVE"
                                            ):
                                                has_active_cogc_program = True
                                                break

                                    if (
                                        has_active_cogc_program
                                        or planet_candidate.HasLocalMarket
                                        or planet_candidate.HasChamberOfCommerce
                                    ):
                                        is_planet_suitable_for_building = True
                                    else:
                                        simulation_log.append(
                                            f"  Planet {planet_candidate.PlanetName} ({planet_candidate.PlanetId}) is not ideal for {building_cogc_category} building {sub_bldg_template.building_ticker}."
                                        )
                                else:  # If no specific COGC category or unrecognized, assume generally suitable
                                    is_planet_suitable_for_building = True
                                    simulation_log.append(
                                        f"  Planet {planet_candidate.PlanetName} ({planet_candidate.PlanetId}) is considered generally suitable for building {sub_bldg_template.building_ticker} (category not specific or recognized)."
                                    )
                                ...
                                if planet_candidate.PlanetId not in planet_recommendations_map:
                                    ...
                                    simulation_log.append(
                                        f"  Created new RecommendedPlanet entry for {planet_candidate.PlanetName} ({planet_candidate.PlanetId}) with placeholder site ID {planet_recommendations_map[planet_candidate.PlanetId].site_id}."
                                    )
                                ...
                                if units_that_fit_in_remaining_area > 0:
                                    ...
                                    simulation_log.append(
                                        f"  Found new suitable planet {current_planet_rec.planet_name} ({chosen_planet_id_for_building}) for {sub_bldg_template.building_name}."
                                    )
                                    break  # Found a planet, break from inner loop

                            if chosen_planet_id_for_building is None:
                                simulation_log.append(
                                    f"  WARNING: No suitable planet found for remaining {remaining_amount_to_place} units of {sub_bldg_template.building_name}. Skipping these units."
                                )
                        ...
                        if units_to_place_on_this_planet == 0:
                            simulation_log.append(
                                f"  WARNING: No space for {sub_bldg_template.building_name} on {current_planet_rec.planet_name}. Moving to next planet search."
                            )
                        ...
                        current_planet_rec.recommended_buildings_on_planet.append(
                            RecommendedBuilding(
                                ...
                                estimated_cost_credits=sub_bldg_template.estimated_cost_credits
                                * units_to_place_on_this_planet,
                                planet_id=chosen_planet_id_for_building,
                                site_id=current_planet_rec.site_id,
                            )
                        )
                        current_planet_rec.total_estimated_cost_credits_on_planet += (
                            sub_bldg_template.estimated_cost_credits * units_to_place_on_this_planet
                        )
                        overall_total_estimated_cost += (
                            sub_bldg_template.estimated_cost_credits * units_to_place_on_this_planet
                        )

                        # Accumulate workforce demand for the placed buildings on this planet
                        bldg_workforce_reqs = initial_state.static_building_workforces.get(
                            sub_bldg_template.building_ticker, []
                        )
                        for req in bldg_workforce_reqs:
                            ...
                            current_planet_rec.total_estimated_workforce_needed_on_planet[req.workforce_type_name] = (
                                current_planet_rec.total_estimated_workforce_needed_on_planet.get(
                                    req.workforce_type_name, 0.0
                                )
                                + required_amount_for_type
                            )
                            ...
                            overall_total_additional_workforce_demand[req.workforce_type_name] = (
                                overall_total_additional_workforce_demand.get(req.workforce_type_name, 0.0)
                                + required_amount_for_type
                            )
                            simulation_log.append(
                                f"  Planet {current_planet_rec.planet_name}: Building {sub_bldg_template.building_name} ({units_to_place_on_this_planet} units) will demand {required_amount_for_type:.2f} units of {req.workforce_type_name} workforce."
                            )

                        remaining_amount_to_place -= units_to_place_on_this_planet
                        simulation_log.append(
                            f"  Placed {units_to_place_on_this_planet} units of {sub_bldg_template.building_name} on {current_planet_rec.planet_name}. Remaining to place: {remaining_amount_to_place}."
                        )

            else:
                simulation_log.append(
                    f"  WARNING: Production rate per building for {desired_material_ticker} is 0 or not determined. Cannot scale production chain recommendations."
                )

        elif source_type == "BUY":
            simulation_log.append(
                f"  Optimal strategy for {desired_material_ticker}: BUY from market. Estimated cost: {final_cost_per_unit:.2f} ICA/unit."
            )
            overall_total_estimated_cost += final_cost_per_unit * production_gap  # Cost to buy the required quantity
            simulation_log.append(
                f"  Total cost to buy {production_gap:.2f} units of {desired_material_ticker}: {overall_total_estimated_cost:.2f} ICA."
            )
            pass
        else:
        ...
        simulation_log.append(
            f"Current production of {desired_material_ticker} ({current_prod:.2f}) already meets or exceeds target ({target_production_rate:.2f}). No expansion recommended."
        )
    ...
    for planet_id, planet_rec in planet_recommendations_map.items():
        ...
        for (
            wf_type_ticker,
            amount_needed_on_planet,
        ) in planet_rec.total_estimated_workforce_needed_on_planet.items():
            workforce_type_name = wf_type_ticker

            simulation_log.append(
                f"    Workforce Type '{workforce_type_name}' demand on {planet_rec.planet_name}: {amount_needed_on_planet:.2f} units."
            )

            if amount_needed_on_planet <= 0:
                simulation_log.append(
                    f"    No demand for {workforce_type_name} on {planet_rec.planet_name}. Skipping housing for this type."
                )
                continue
            ...
            for hb_ticker, hb_capacities in HOUSING_BUILDING.items():
                ...
                if not static_hb_def:
                    simulation_log.append(
                        f"    WARNING: Static building definition not found for hardcoded housing ticker '{hb_ticker}'. Skipping."
                    )
                    continue
                ...
                if current_hb_capacity_for_type > 0:
                    ...
                    for cost_item in housing_cost_items:
                        if initial_state.dynamic_market_data:
                            cost_market_data = next(
                                (
                                    md
                                    for md in initial_state.dynamic_market_data
                                    if md.MaterialTicker == cost_item.MaterialTicker
                                ),
                                None,
                            )
                            item_price = 0.0
                    ...
                    cost_per_unit_wf = (
                        estimated_cost_per_housing_building / current_hb_capacity_for_type
                        if current_hb_capacity_for_type > 0
                        else float("inf")
                    )
                    area_per_unit_wf = (
                        area_per_housing_building / current_hb_capacity_for_type
                        if current_hb_capacity_for_type > 0
                        else float("inf")
                    )

                    suitable_housing_candidates.append(
                        (
                            static_hb_def,
                            cost_per_unit_wf,
                            area_per_unit_wf,
                            current_hb_capacity_for_type,
                        )
                    )
            ...
            if suitable_housing_candidates:
                ...
                if capacity_per_housing_unit <= 0:
                    simulation_log.append(
                        f"    WARNING: Selected housing building '{housing_bldg_def.name}' provides 0 capacity for '{workforce_type_name}'. Skipping recommendation for this workforce type on {planet_rec.planet_name}."
                    )
                    continue
                ...
                simulation_log.append(
                    f"    Planning {num_housing_buildings_to_build} units of housing '{housing_bldg_def.name}' to provide {num_housing_buildings_to_build * capacity_per_housing_unit:.2f} {workforce_type_name} capacity on {planet_rec.planet_name}."
                )
                ...
                planet_rec.total_estimated_cost_credits_on_planet += (
                    estimated_cost_per_housing_building * num_housing_buildings_to_build
                )
                overall_total_estimated_cost += estimated_cost_per_housing_building * num_housing_buildings_to_build

                simulation_log.append(
                    f"    Housing buildings will consume {num_housing_buildings_to_build * (housing_bldg_def.area if housing_bldg_def.area is not None else 0.0):.2f} additional area on {planet_rec.planet_name}."
                )
                ...
                simulation_log.append(
                    f"    WARNING: No suitable housing building found in HOUSING_BUILDING for workforce type '{workforce_type_name}'. Cannot meet demand on {planet_rec.planet_name}."
                )
    # --- END HOUSING LOGIC ---
    ...
    for planet_id, planet_rec in planet_recommendations_map.items():
        ...
        if site_area_needed_on_this_planet > max_possible_area_on_planet:
            simulation_log.append(
                f"  WARNING: Planet {planet_rec.planet_name} ({planet_rec.planet_id}): Total area needed ({site_area_needed_on_this_planet:.2f}) exceeds maximum possible area ({max_possible_area_on_planet:.2f}) even with all permits. This site is over-capacity."
            )
            ...
            if area_deficit_from_base > 0:
                ...
                simulation_log.append(
                    f"  Planet {planet_rec.planet_name} ({planet_rec.planet_id}): Sufficient area with base permit."
                )
        ...
        simulation_log.append(
            f"  Planet {planet_rec.planet_name} ({planet_rec.planet_id}): Recommending {permits_to_buy_on_this_planet} permit(s). Permit Cost: {permits_to_buy_on_this_planet * PERMIT_COST:.2f} ICA."
        )
